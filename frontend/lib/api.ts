...
export type ApiReview = { id: number; authorName: string; rating: number; title: string; body: string; productName?: string; isVerified: boolean };
...
export function toProduct(p: ApiProduct): Product & { id: number } {
...
// ── Server-side fetch (Next.js cache-aware) ───────────────────────────────────
const SERVER_BASE = (process.env.API_URL ?? process.env.NEXT_PUBLIC_API_URL ?? 'http://localhost:3000/api').replace(/\/$/, '');
